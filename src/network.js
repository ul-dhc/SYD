...
function definitionsMarkup() {
  ...
  const patterns = TYPE_ORDER.map((type, index) => `<pattern id="pencil-${type}" patternUnits="userSpaceOnUse" width="5" height="5" patternTransform="rotate(${angles[index]})"><rect width="5" height="5" fill="var(--background)"></rect><path d="M 0 1 L 5 1 M 0 3.7 L 5 3.7" stroke="var(--node-${type})" stroke-width=".75" opacity=".82"></path></pattern>`).join("");
